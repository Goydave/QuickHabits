  ...
  useEffect(() => {
    if (isOpen) {
      ...
      generateAudioJournalingPrompt()
        ...
        .catch(err => {
          console.error("Failed to generate audio prompt:", err);
          setError("I had trouble coming up with a prompt. Please try again in a moment.");
          toast.error("Failed to generate audio prompt.");
        })
        .finally(() => {
          setIsLoading(false);
        });
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Listen to the prompt below, then take a moment to reflect and write.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="my-8 flex items-center justify-center text-center">
          {isLoading ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-muted-foreground">Generating your unique prompt...</p>
            </div>
          ) : error ? (
